...
CREATE TYPE status_usuario AS ENUM ('offline', 'online');
CREATE TYPE tipo_conversa  AS ENUM ('privado', 'grupo');
CREATE TYPE funcao_membro  AS ENUM ('membro', 'administrador');
CREATE TYPE plataforma_push AS ENUM ('android', 'ios');
CREATE TYPE tipo_mensagem AS ENUM ('texto', 'arquivo', 'video');
...
CREATE TABLE conversas (
    id         UUID          PRIMARY KEY DEFAULT gen_random_uuid(),
    tipo       tipo_conversa NOT NULL DEFAULT 'privado',
    nome       VARCHAR(100),
    descricao  TEXT,
    url_foto   TEXT,
    ...
    CONSTRAINT chk_direto_sem_nome CHECK (tipo != 'privado' OR nome IS NULL)
);
...
CREATE TABLE membros_conversa (
    ...
    funcao      funcao_membro NOT NULL DEFAULT 'membro',
    saiu_em     TIMESTAMPTZ,
...
CREATE INDEX idx_mensagens_busca_texto ON mensagens USING gin (to_tsvector('portuguese', conteudo))
    WHERE tipo = 'texto' AND excluida = FALSE;
